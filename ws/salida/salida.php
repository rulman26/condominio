...
class salida 
{
  ...
  function ResumenVenta($fecha){
    $pdo = baseDatos::conectar();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT SAL.SALIDA_ID,SAL.SALIDA_FECHA,DATE_FORMAT(SAL.SALIDA_FECHA,'%d/%m/%Y %H:%i') FECHA,
      ITM.ITEM_NOMBRE,ITMTIPO.ITEM_TIPO_NOMBRE PRESENTACION,
      LAB.LABORATORIO_NOMBRE,SAL.SALIDA_PRECIO_COMPRA,SAL.SALIDA_PRECIO,SAL.SALIDA_CANTIDAD,
      ROUND(SAL.SALIDA_PRECIO*SAL.SALIDA_CANTIDAD,2) TOTAL,
      ROUND((SAL.SALIDA_PRECIO-SAL.SALIDA_PRECIO_COMPRA)*SAL.SALIDA_CANTIDAD,2) GANANCIA    
      FROM SALIDA SAL
      ...
      WHERE DATE_FORMAT(SAL.SALIDA_FECHA,'%d/%m/%Y')=? ORDER BY 2";
    $q = $pdo->prepare($sql);
